...
| Local sale drove stock negative | Sale **accepted** and flagged `RequiresReview` with `sync.sold_into_negative_stock`, at a location whose policy is `AllowOfflineWithReview`. FEFO allocates what the shelf can cover and puts the remainder on the batch it finished on — the latest-expiring, the one most likely to be the stock actually standing there, because a shelf holding more than the system says is usually a receipt nobody recorded. When no batch is known at all the shortfall names none rather than inventing one. The bucket goes negative and is left saying so: a balance held at zero is how a count that never happens starts. A `NegativeStockAttempt` is recorded whether the draw was permitted or refused — a permitted oversell is the one somebody most needs to find, because it is the shelf that is now wrong. A location that left the policy at `Prohibit` still refuses, and the device escalates and keeps the event (§3.2); an **online** sale is unchanged, because there is a terminal in front of it and the cashier can be told. |

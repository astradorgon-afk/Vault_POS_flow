...
- [~] **Phase 13 — Synchronization:** outbox, push/pull endpoints, idempotency behaviour, retries, conflict rules
  ...
  - [~] C38 — device sale tables and adapters: `local_sale`/`local_sale_item`/`local_payment` from the server's own configurations (migration `DeviceLocalSale`); `DeviceSalesRepository` (projects `SaleProduct` from the cache, enqueues `SaleCompleted`), `DeviceCustomerRepository` (answers null, so a named-customer sale is refused offline), `DeviceExpiryService` (FEFO from stock the register holds). `CompleteSaleCommand` stays `Pending`: end-to-end it refused `inventory.insufficient_stock` against stock the register provably held. The ledger's enlisted path was the first suspect and is now covered and passing, so the fault is in the sale wiring (STATUS.md §5).

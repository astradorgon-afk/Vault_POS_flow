...
| Tests | **1,121 passing without PostgreSQL: Domain 378, Application 247, Infrastructure 245, Security 52, Architecture 25, API 174** (2026-09-17, through C41 offline void and reprint). PostgreSQL tests require Docker; the suites ran in Release through C32 on a machine with Docker (Infrastructure 217 passed, API 176 passed, 0 skipped), including all 21 PostgreSQL tests — after an earlier run under heavy load had silently skipped the 18 Infrastructure ones (see §4). C33–C35 have not been run against PostgreSQL; they add no PostgreSQL migration. |
| Migrations | 28 PostgreSQL migrations plus 9 independent SQLite device migrations, all forward-only. The device migrations are exercised against encrypted SQLCipher storage. |
...
1. **Returns and refunds, the last POS entries.** Both are declared
   offline-capable for a sale the device holds locally. They need
   `local_sales_return` and the refund rows, and they carry a consequence to
   handle in the same change rather than after it: the moment a device can refund
   cash, `DeviceShiftRepository.GetShiftCashTotalsAsync` must stop reporting zero
   refunds, or every drawer that paid one out will look short by exactly the
   amount refunded.

...
| Tests | **1,209 passing without PostgreSQL: Domain 378, Application 247, Infrastructure 308, Security 52, Architecture 25, API 199** (2026-09-17, through C53 the failure queue). PostgreSQL tests require Docker; the suites ran in Release through C32 on a machine with Docker (Infrastructure 217 passed, API 176 passed, 0 skipped), including all 21 PostgreSQL tests — after an earlier run under heavy load had silently skipped the 18 Infrastructure ones (see §4). C33–C53 have not been run against PostgreSQL; C43 and C50 add the only PostgreSQL migrations among them. |
...
Pos.Api.IntegrationTests    199 passing   endpoints through the real pipeline (SQLite), including customer lifecycle, permissions, audit behavior, discount enforcement, the web-terminal checkout surface, sale-lifecycle read routes, the payment-mix checkout flows, the expired-batch override contract, scheduled-price cancellation, and a register uploading a shift and a sale it rang up through an outage, priced from a row since superseded, reprinted and then voided, and goods taken back and refunded
```

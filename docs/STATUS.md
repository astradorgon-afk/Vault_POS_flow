...
| Phases remaining | None. All 18 phases complete through C73. No Phase 19 planned. |
...
**Every entry says which source it came from.** A person chasing a discrepancy
needs to know whether they are looking at something somebody did, something the
ledger posted, or something head office decided about an upload. Those read very
...
**A posting shows every leg, not just the side that was asked about.** Stock
leaving one bucket always arrives somewhere; a chain showing one side would look
like stock vanishing. Legs carry their value only for a caller with the financial
permission.
...
reading the original posting with no sign it had been reversed — which is exactly
the reading that counts the same loss twice.

**A document nothing in the caller's stores touched is a 404**, not an empty
timeline. "You may not see this" and "nothing happened" are different answers, and
an empty one would quietly tell a manager the second.

**Phase 16 is complete.** The overview, the exception board and the drill-down are
all API surface; the dashboard's web UI is client work and is not built here.
...
**Named ranges resolve in a real timezone**, named in the response so a reader can
tell whose day they are looking at: one store's own when a store is named, the
organization's otherwise. "Today" for a business in Manila is not the UTC day, and
a dashboard answering in UTC would show a store, at nine in the morning, a fraction
of the day it had already had — the kind of wrong that gets explained away rather
than reported. The resolver is its own domain type with nineteen tests, because
every case in it is an off-by-one somebody would otherwise find in a comparison
and quietly distrust: `Last7` is seven days inclusive, `PrevMonth` ends on a leap
February's 29th, a quarter starts on its own first month, and `Custom` with a
missing date is refused rather than falling back to today.

**The financial half is withheld, not zeroed.** Without `report.view.financial`,
cost, gross profit, margin and the stock value come back null. Zero reads as "we
made nothing", which is a statement about the business rather than about the
reader. One route serves both, because a second would be a second place for the
arithmetic to drift.

**The stock panel is a snapshot of now**, whatever period the sales figures cover,
and carries its own `asOfUtc` to say so. Mixing the two silently is how somebody
concludes last month's sales emptied a shelf that was restocked on Tuesday. Out of
stock and low are counted separately: one is a sale being refused right now and the
other is a sale that will be refused next week, and they go to different people.
`shareOfRevenue` is null rather than zero when nothing sold anywhere — a share of
nothing is not nought per cent — and the store comparison is never truncated,
because a league table missing its tail lies about who is last.

**A latent flake from C55 surfaced and is fixed.** `SyncBaselineProcessor` read
`DateTimeOffset.UtcNow` directly to decide whether a price was still effective and
whether a snapshot had expired, so its test passed in the morning and failed in the
afternoon. It takes `ISystemClock` now, like everything else that asks the time.

---

C65 lets the reports leave the building. `GET /api/v1/reports/{report}/export`
returns CSV — a GET rather than the POST the API plan named, because it reads and
returns and creates nothing.

**`report.export` gets you the file format, not the report.** Each name is checked
against the permission its own route requires before a row is read. Without that,
the export route's single permission would have been a way around
`report.view.financial`: anybody who could export anything could export the
valuation. The name-to-permission table lives in one place, so adding a report to
the export list without deciding who may read it is a compile error rather than an
open door.
...
text" and strip on display. Values go out invariantly (round-trip timestamps, `.`
decimal separator), because an export formatted for the server's locale changes
meaning when the server moves, and a header row is always written so an empty
period downloads a file that says what the columns were.

The writer is by hand rather than a dependency: the rows are flat records of
primitives, and the whole job is quoting and that guard.

**XLSX is not built.** It needs a third-party spreadsheet library, which is a
dependency decision rather than a reporting one, and CSV opens in every
spreadsheet there is. The roadmap row stays `[~]`.

**A gap in the spec turned up while writing the tests.** `report.export` is granted
to `Auditor` alone in the role bundles, and the role matrix in PERMISSIONS.md has
no row for it at all — so an Owner who may read every report cannot export any of
them. Nobody in the bundles holds export without also holding the financial
permission, which meant the gate's own test had to stand its caller in that gap
with a real `UserPermissionOverride`. Who should be able to export is a business
question and is left open rather than answered by widening the grants.

---

C64 covers who did what, what turned up unauthorised, and what is about to go off.
`/audit` lists recorded actions under `audit.view`, `/unauthorized-inventory` the
quarantine incidents, and `/expiry` the stock that has expired or is about to.
...
which is what the actor held at the time rather than what they hold now.

**An audit entry with no location is business-wide**, and reaches only a caller who
is not scoped to particular stores. A role change or a permission grant shown to a
store manager would leak head-office activity through a report about their own
shop; hiding it from an owner would lose the entries that matter most. That is a
genuine asymmetry rather than an oversight, and a test asserts both halves.

`daysOpen` on an incident ages an open one to now and keeps how long a closed one
took, so a single number answers both "how long has this been sitting" and "how
long did that take". `unidentifiedLines` counts the lines whose goods the catalogue
does not know: unidentifiable stock is the most worth investigating, and the test
identifies the recognised lines through the domain's own `Identify` rather than
setting a column, so the count is proved to read the right thing.
...
next week, not less. A bucket holding nothing is left out; there is nothing there
to throw away.

**The sync-problems report the API plan named already exists** as
`GET /api/v1/sync/failures` (C53), reading the server's own verdicts. It is
deliberately not duplicated under `/reports`: a second surface over the same rows
could only add a way for the two to disagree.

---

C63 reports what went wrong with the stock. `/adjustments` summarises what was
written off or corrected by why, `/shrinkage` values the same losses behind
`report.view.financial`, and `/count-variance` lists the physical count lines that
did not match the system.

**Which movement types count as a loss is declared once, in code.** Inferring it
from the sign of a leg would have been shorter and wrong: a transfer dispatch and a
count correction both reduce a bucket, and only one of them is stock the business
no longer has.

**A count correction is an adjustment and not shrinkage.** It says the books were
wrong, not that goods left the building. Folding it in would let a business shrink
its shrinkage by counting more often, which is the sort of metric that gets gamed
without anybody deciding to game it. It appears in `adjustments` and not in
`shrinkage`, and a test asserts both halves of that.

**Shrinkage counts only the legs that took stock away.** Stock put back by an
approved adjustment shows as `quantityIn` in the adjustment report and never as a
...
closed month's figure every time a supplier changed a price, and would stop
reconciling to the accounts it exists to explain.

**A count line nobody counted is not a variance of zero.** It reports a null
physical quantity and a null variance, because "we looked and it was right" and
...
Writing the fixture ran into three domain rules, each of them right: a write-off
needs an approver and a reason, a count adjustment must cite an inventory count
rather than a stock adjustment, and a write-off cannot be reversed by flipping the
signs on its own movement type. The test was changed to match the domain rather
than the other way round.

Still open in these two roadmap rows: a dedicated expiry report, and
unauthorized-inventory and quarantine reporting.
...
rolls dispatched transfers up by the lane they travelled.

**A transfer is in scope when *either* end is.** This is the one report whose
scope is not a single location column, and the obvious implementation — filter on
the source — would have hidden every incoming shipment from the people waiting for
it. A test asserts both ends see the same transfer and a third store sees nothing.

**The three quantities stay three columns.** What was asked for, what actually
left the source, and what the destination counted are three separate records of
three separate human acts; collapsing them is how a shipment that arrived two
cartons short comes to look complete. `daysInFlight` is null once a transfer has
been received, because there is nothing in flight to count, and the rows sort by
it descending — what is stuck is what somebody opens this report to find.

**Distribution counts on dispatch, over the dispatch date.** Counting on receipt
would make a lane look idle while a fortnight of stock sat in a van; counting over
the creation date would file this month's shipment under the month it was
requested. `shortfallQuantity` includes stock still legitimately in transit,
because the number a lane is judged on is what has not arrived yet.

The tests drive real transfers through Submit, Review, Approve, Pick, Ready,
Dispatch and Receive rather than writing the columns a report reads. A fixture
that set those columns directly could pass against a lifecycle the domain would
refuse, which on a report about a workflow is the failure worth avoiding.
...
can walk to the till. It reads `sync.processed_event` rather than a second table
devices report into: everything on it is the server's own decision, so the record
already exists and a reporting round-trip could only add a way for the two to
disagree. What the server genuinely cannot see are the events that never reached
it, and the register's own banner already counts those.

`POST /api/v1/sync/failures/{eventId}/retry` deliberately re-applies nothing. A
refused event was refused because of something true at the time, and asking again
unchanged earns the same answer; it becomes worth asking when a person changes
that thing. So the route records that they did, as a `SyncRetryRequested`
directive on that register's own feed — a register that is offline cannot be told
anything at all, and the feed is already what it comes back to.

The register reopens only an event that had actually stopped. One still waiting
its turn keeps its backoff rather than having it thrown away, and one already
accepted is never reopened: asking a register to send a sale head office already
holds is how a day's takings get counted twice. The test pushes all three states
through one page and checks each.

C54 runs a real register against the real server over HTTP: it downloads its
catalogue, loses the line, opens a shift, sells, comes back, uploads what it did,
and hears about the price head office changed while it was away. Every piece had
its own tests; what this covers is the **joins**, and that is where it found
things.

`ChangeFeedDownloader` is itself one of those joins, and it did not exist — C51
built a route, C29 built an applier, and nothing carried a page from one to the
other. A change kind this build does not know is refused rather than skipped: a
register that quietly ignores half a feed and believes its catalogue is current
is the worse failure by some distance. PIN sign-in now puts the cashier's offline
authority on the register's feed, which nothing had ever done — a register could
download a whole catalogue and still refuse to sell, because it had nothing to
answer a permission question with once the line was gone.

**Three bugs that only a round trip could find:**

- **`EXT-CUSTOMER` was scoped to itself.** Every register posts the other leg of
  a sale against it, and C50 scoped every location change to its own location. So
  no register ever received it, and none could have sold anything. Counterparty
  locations are global now.
- **The feed's currency came from an organization row that is never written.**
  Every `LocationChanged` arrived with an empty currency and the device refused
  the page. The currency belongs to the money — a price carries its own, and the
  system's single currency now lives on `Money` where it is a fact rather than a
  lookup.
- **The download outcome could not tell "nothing to do" from "the page was
  refused".** Both returned zero applied and no flags. It carries the reason now,
